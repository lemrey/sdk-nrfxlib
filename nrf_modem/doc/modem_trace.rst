...
.. note::
	...
	If no trace data is available, the :c:func:`nrf_modem_os_trace_get` function blocks until new data becomes available or the modem is shut down or faulted.
...
Hence, the :c:func:`nrf_modem_trace_processed_callback` function must be called with the total number of bytes in the trace fragments before making a new call to the :c:func:`nrf_modem_os_trace_get` function.
